...
function cwMessageTransfer() {
  ...
  //「テスト」環境でのトリガー解除
  let test = mcSheet.getRange("G1").getValue();
  ...
  //管理表への追加
  let lastRow = cwSheet.getRange("A1").getNextDataCell(SpreadsheetApp.Direction.DOWN).getRow(); //転記元の最終行を確認、取得する。
  ...
  let before_lastRow_mc = mcSheet.getRange("B2").getNextDataCell(SpreadsheetApp.Direction.DOWN).getRow();
  ...
  /*msgId = 投稿日時 が書かれたセル番号を取得（以下、同じ）
    ...
    machineCode = 機械コード
    wpCode = 対象職場（職場コード）
    notAppMan = 作業者除外（対象外とする作業者）
    notAppLine = 工程除外（対象外とする工程）
    finProcess = 完成工程（最終工程化否かを判断）
  */
  ...
  for (let i = 1; i < cwData.length; i++){
    ...
    const Acount = rowCw[cw_col_acount - 1];  //CW転記の「アカウント」の値取得
    const Msg = rowCw[cw_col_msg - 1];  //CW転記の「メッセージ」の値取得
    ...
    const machineCode = rowCw[cw_col_machineCode - 1];  //CW転記の「機械コード」の値取得
  ...
  //作業者交替一覧表に更新があった場合に、通知メールを送信する。
  if(after_lastRow_mc > before_lastRow_mc){
    
    //更新された管理Noをメール側へ転記する。（更新No.〇〇～）
    const updateNo = mcSheet.getRange(startRow,1).getValue();

    //通知メールを送るGAS
    const mailSheet_A = ss.getSheetByName("通知メール");
    mailSheet_A.getRange("E41").setValue(updateNo);

    //メール本文データの読み込み
    const title = mailSheet_A.getRange("B1").getValue();
    const mText = mailSheet_A.getRange("B40").getValue();
    const add = mailSheet_A.getRange("B41").getValue();
    
    const mail_address = add;
    const mail_title = title;
    const mail_main = mText;

    //メール作成
    MailApp.sendEmail(mail_address,mail_title,mail_main);
    //正常に終了した場合をログに記録。
    console.log("変化点が検出されました。処理は正常に終了しました。");
